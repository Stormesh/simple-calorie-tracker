...
<template>
  <div
    :class="
      nutrition.max && Math.round(nutrition.value) > nutrition.max ? 'stat-card-over' : 'stat-card'
    "
  ...
  >
    ...
    <div v-if="nutrition.max" class="flex items-baseline gap-1 mt-0.5">
      <span class="text-xs text-white/30 font-mono"
        >/ {{ nutrition.max }}<span class="text-[10px]">{{ nutrition.unit }}</span></span
      >
    </div>
    <span class="text-xs font-semibold text-gaming-400 uppercase tracking-widest mt-1.5">{{
      nutrition.label
    }}</span>
    <div v-if="nutrition.max" class="w-full mt-2 h-1 rounded-full bg-gaming-950/80 overflow-hidden">
      <div
        ...
        :class="
          Math.round(nutrition.value) > nutrition.max
            ? 'bg-linear-to-r from-hp-red to-red-600'
            : 'bg-linear-to-r from-gaming-500 to-gaming-400'
        "
        :style="{
          width: `${Math.min((nutrition.value / nutrition.max) * 100, 100)}%`,
        }"
      />
    </div>
  </div>
