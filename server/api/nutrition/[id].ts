...
const parseServings = (servings: {
  ...
  const servingArray = Array.isArray(servings.serving) ? servings.serving : [servings.serving];
  ...
  const parsedServings = servingArray.map((serving) => {
    ...
    for (const key of nutrientKeys) {
      if (key in newServing) {
        (newServing as Record<string, string | number>)[key] = parseNutrient(newServing[key]);
      }
    }
    ...
    if (useOAuth2) {
      ...
      detailData = await $fetch<IFatSecretDetailResponse>(FATSECRET_API_URL, {
        ...
        body: new URLSearchParams(
          Object.fromEntries(
            Object.entries(detailParams).map(([key, value]) => [key, String(value)]),
          ),
        ).toString(),
      });
    } else {
